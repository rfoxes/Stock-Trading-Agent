...
- **Same-day DOUBLE-FIRE → detect it and do NOT re-execute (observed 2026-07-07 and 2026-07-08).** The scheduled task has twice fired a second time on the same session (7/7: 09:09 + 16:03; 7/8: 16:03 + 16:51). The second fire re-reads state files that the FIRST fire (or an intervening research run) already advanced — so a naive second run will re-do the day's work. **The tell:** `market-status` `now_iso` shows the same calendar day as a `last_handoff.md` that already narrates a *completed* run for that day, AND git log already has a `[trader YYYY-MM-DD]` commit for today. **Confirm with the broker:** if the first fire submitted orders, `get_open_orders()` (client-direct, bypassing the buggy CLI formatter) shows them still resting with the first run's order-IDs/timestamps, positions unchanged, cash to the penny. **Correct response: take NO trading action.** Specifically DO NOT run `cli execute` — most library strategies (incl. `equity_event_driven_catalyst`, whose `evaluate()` comment states "THE STRATEGY IS THE STOP... no resting broker stop order") key off `ctx.positions`, NOT resting broker orders, so they will RE-EMIT the same exits against still-open positions → duplicate resting sells that OVERSELL into short positions when both sets fill at the next open. Also skip log-closed (nothing filled) and skip re-triage (unclaimed already 0). Record the double-fire in the handoff, PRESERVE the prior run's forward plan (don't clobber the next-run reconciliation instructions), flag the schedule to the operator, and git-sync. This is distinct from a broker wipe (that's flat book + unchanged cash + no closes); a double-fire leaves the book fully intact and the prior run's orders resting.
- **Reconcile resting-exit fills from the ACTUAL broker fill price, NOT prior-day mark estimates (observed 2026-07-09).** When a strategy submits resting exits post-close (e.g. `equity_event_driven_catalyst`'s time/ATR stops) that fill at the next open, the next run must `log-closed` them — and the `pnl_fraction` MUST come from the real fill, not the prior handoff's mark-based guess. Pull it client-direct: `AlpacaClient(Settings(_env_file='.env')).get_order(<order_id>)` returns `filled_avg_price`/`filled_qty`/`filled_at`/`status` (the order IDs are in the prior run's execute output / handoff); `pnl_fraction = (filled_avg_price − avg_entry) / avg_entry`. **Why this matters:** on 7/9 the 7/8 marks estimated MU at −3.75% (a loss), but MU gapped +8% at the open on its $250B-capex news and the time stop sold into strength → the real fill was **+2.96%, a WIN**. Logging the estimate would have flipped MU's win/loss record and skewed the strategy's Sharpe. `log-closed` feeds strategy stats permanently, so an estimated sign error is not cosmetic. Cross-check: the sum of `filled_avg_price × qty` across the exits should equal the cash rise to the penny — if it does, it's a confirmed legit close (not a wipe) AND your reconciliation is arithmetically complete.
- **SKIPPED-RUN / multi-day coverage gap → NOT a wipe, resume normally (observed 2026-07-21).** The 7/21 run was the first coverage since 7/16 — the 7/17 and 7/20 trader/news runs and the Saturday 7/18 research run all failed to fire (no commits for any of them). This is the **third and benign** member of the schedule-anomaly trio, distinct from the double-fire (7/7, 7/8) and the transient wipe (7/7). **Its signature:** book **INTACT** (every held position at its exact prior qty/avg-entry), cash unchanged to the penny, **no `trade_closed` events**, and a multi-day hole in git log with no `[trader]`/`[news]` commits. **Why it's safe:** because no `cli execute` ran during the gap, nothing algorithmic happened — held names simply rode their own strategy rules (the strategy is evaluated only at execute time; there are no resting broker stops for most strategies), so nothing opened or closed. **Correct response: resume the standard workflow — do NOT freeze, do NOT `log-closed` (nothing closed), reconcile nothing.** Distinguish it cleanly from a wipe: the wipe/freeze doctrine keys on a **FLAT** book (positions gone) + unchanged cash; a skipped-run gap has an **intact** book + unchanged cash, so "cash unchanged" alone is NOT a wipe signal when the positions are still present. Two consequences to handle on resume: (1) any provisional `revalidate_by` deadlines that lapsed during the gap are now **overdue-but-still-quarantined** — the trader does NOT validate them (research owns validation), just keeps them quarantined and flags the operator (on 7/21: QCOM/SPCX/SYNA all hit `revalidate_by 2026-07-21` with no Saturday research); (2) multiple sessions' worth of news promotions arrive at once — triage every accumulated unclaimed name normally (7/21: NBIS/IREN/AMD, universe 37→40). The provisional book only shrinks when research runs, so repeated research skips make it grow unbounded — worth an operator escalation path.
