"""Position Trading Agent — operates on weekly bars.

Holds positions for 3+ weeks, focusing on longer-term trends and sector rotation.
...
MARKET_ANALYSIS_PROMPT = """You are a quantitative position analyst operating on weekly bars with 3+ week holding periods.
...
1. Long-term trend via SMA 50 and SMA 200 (golden/death cross status)
2. Sector rotation signals and relative strength
3. Long-term momentum and trend persistence (ADX)
4. Major support/resistance from weekly price structure

Provide a concise analysis covering:
1. Primary trend direction and strength
2. SMA 50/200 relationship and crossover status
3. Sector positioning and relative momentum
4. Recommended bias: BULLISH, BEARISH, or NEUTRAL

Respond in JSON format:
{{"bias": "bullish|bearish|neutral", "confidence": 0.0-1.0, "key_levels": {{"sma_50": X, "sma_200": Y, "support": Z, "resistance": W}}, "trend_status": "...", "reasoning": "..."}}
"""

STRATEGY_SELECTION_PROMPT = """You are a quantitative strategy selector for position trading (3+ week holds).
...
Select the most appropriate position trading strategy for the current conditions.
Consider trend following, sector rotation, and momentum strategies.
Respond in JSON format:
{{"strategy_id": "...", "reasoning": "...", "expected_edge": "..."}}
"""


def create_position_agent(
    settings: Settings,
    market_data: MarketDataService,
    knowledge_base: KnowledgeBaseClient,
    safety_gate: SafetyGate,
) -> StateGraph:
    """Create the position trading agent subgraph."""
    ...
    def market_analysis_node(state: TradingAgentState) -> dict:
        """Fetch weekly bars and compute long-term indicators for the watchlist."""
        log = logger.bind(agent="position", node="market_analysis")
        ...
        for symbol in watchlist[:10]:  # Limit to 10 symbols per cycle
            try:
                df = market_data.get_bars(symbol, "1Week")
                if df.empty:
                    continue
                ...
                # Summarize latest indicators with position trading focus
                ...
                if sma_50 > 0 and sma_200 > 0:
                    summary["sma_cross_status"] = "golden_cross" if sma_50 > sma_200 else "death_cross"
                else:
                    summary["sma_cross_status"] = "insufficient_data"
                ...
                try:
                    ...
                    response = analysis_llm.invoke([
                        SystemMessage(content="You are a quantitative position analyst. Respond only in valid JSON."),
    ...
    def strategy_selection_node(state: TradingAgentState) -> dict:
        """Select the best position trading strategy for current conditions."""
        log = logger.bind(agent="position", node="strategy_selection")
        ...
        # Query knowledge base for position trading strategies
        try:
            strategies = knowledge_base.search_strategies(
                query=f"position trading trend following sector rotation momentum strategy for {state.get('market_regime', 'unknown')} market",
                n_results=5,
        ...
        try:
            ...
            response = strategy_llm.invoke([
                SystemMessage(content="You are a position trading strategy selector. Respond only in valid JSON."),
                HumanMessage(content=prompt),
            ])
    ...
    def risk_evaluation_node(state: TradingAgentState) -> dict:
        """Evaluate risk with wider stops and larger position sizing for position trades."""
        log = logger.bind(agent="position", node="risk_evaluation")
        ...
        for signal in state.get("signals", []):
            ...
            # Position sizing: wider stops, larger positions for position trades
            ...
            # Half Kelly with higher max position for position trades
            kelly_f = half_kelly(win_rate=0.55, avg_win=0.08, avg_loss=0.05)
            position_max_pct = min(settings.MAX_POSITION_SIZE_PCT * 1.5, 0.15)
            shares = position_size(
                kelly_f=kelly_f,
                portfolio_value=equity,
                price_per_share=price,
                max_position_pct=position_max_pct,
            )

            if shares <= 0:
                continue

            # Position trade stop: 5% max loss per trade (wider stop)
            atr = market_info.get("atr_14", 0)
            stop_distance = min(price * 0.05, atr * 3.0) if atr > 0 else price * 0.05
            ...
            order = OrderRequest(
                ...
                agent_name="position_agent",
                strategy_name=state.get("selected_strategy", {}).get("response", "unknown")[:50],
                reasoning=f"Position signal: {analysis[:200]}",
            )
    ...
    def order_execution_node(state: TradingAgentState) -> dict:
        ...
        log = logger.bind(agent="position", node="order_execution")
