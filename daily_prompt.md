...
- **P0 triage workflow (2026-06-10 doctrine refinement).** For every unclaimed symbol the universe contains, before `cli execute`, run `cli triage-symbol <SYM> [--gap-type <type>]`. The triage either auto-claims with the highest-Sharpe library candidate (Sharpe ≥ 0.5) OR records a `true_library_gap` in `state/library_gaps.md`. EXECUTE WILL ACCEPT library-gap-flagged symbols and refuse any unflagged unclaimed symbol. Never use `cli add-active` to bypass triage — character-match shortcuts are FORBIDDEN. See `manual.md` "P0 — EVERY SYMBOL ALGORITHMICALLY EVALUATED RULE".
